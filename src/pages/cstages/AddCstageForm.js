...
import { useAddNewCstageMutation } from "./cstagesSlice";
...
const AddCstageForm = () => {
    const [addNewCstage, { isLoading }] = useAddNewCstageMutation()
  ...
  const patientId = queryParams.get('patientId');
    ...
    const [presentsym,setPresentsym]=useState([])
  ...
  const handleSelect=(event)=>{ setSelected(event.target.innerText.split(' ')[0]);setCT(event.target.innerText.split(' ')[0]);toggle()}
  ...
  const handleSelected = (selected) => { setPresentsym(selected);  };
  const handleSelectedsitemetas = (selected) => { setSitemetas(selected);  };
  ...
  const canSave = [stage,cT,cN,M,birads,side,datecdx,datemmg,methodbx].every(Boolean) && !isLoading; 
 

  const onSaveCstageClicked = async () => {
    if (canSave) {
        try {
            await addNewCstage({ stage,cnote,cT,ctsize,cN,M,birads,side,patientId,datecdx,presentsym,sitemetas,othermet,othersym,datefirstvst,datedoctorrpt,datemmg,methodbx,otherMethod,htype,grade }).unwrap()
            setDatecdx('')
            setDatemmg('')
            setDatefirstvst('')
            setDatedoctorrpt('')
...
useEffect(() => {
  ...
  <Form>
  <FormGroup row>
  <h4 sm={12}> เพิ่มบันทึก Clinical Diagnostic  <CstagePatient patientId={patientId} /></h4>   
  </FormGroup>
  <FormGroup row>
        ...
        <Col sm={4}>
             <SymptomForm defaultSymptoms={presentsym} othersym={othersym} onOtherSymChange={handleOnOthersymChange}  onSelected={handleSelected} />
        </Col>
  </FormGroup>
  <FormGroup row>
    <Label style={{borderColor:'#2980b9',color:'blue'}}
...
/>
  ...
  <Col sm={4}>    
  <SitemetasForm defaultSitemetas={sitemetas} othermet={othermet} onOtherMetChange={handleOnOthermetChange}  onSelected={handleSelectedsitemetas} />       
              </Col>
  </FormGroup>}
  <FormGroup row>
  <Label 
...
<FormGroup  row >
    ...
    <Button  onClick={()=>navigate(`/patient/${patientId}`)}   >
      ...
      <Button color='primary' onClick={onSaveCstageClicked}  style={{marginLeft: '10px'}}  disabled={!canSave}>
        บันทึก
      </Button>
    </Col>
</FormGroup>

<Modal  isOpen={open??false} toggle={toggle} fullscreen>
        <ModalBody>
        <ListGroup>
